...
import wget
import certifi
from urllib3 import PoolManager as lib3PoolManager
from urllib3 import request as lib3request
    ...
    try:
        for i in range(0,len(years),1):
            ...
            for k in range(0,len(filelist),1):
                ...
                #conn = urllib3.connection_from_url('https://urs.earthdata.nasa.gov')

                print(url + filelist[k])

                http = lib3PoolManager()#cert_reqs='CERT_REQUIRED',ca_certs=certifi.where())

                fields={'username':username,'password':password}

                r = http.lib3request('GET', url + filelist[k],fields)

                file_size = int(r.getheaders("Content-Length")[0])

                print(http,r,file_size)

                #meta = (urlopen(url + filelist[i])).info()
                ...
                #else:
                #
                #    url2Dwl = url + filelist[k]
                #
                #    outFile = outputDir + filelist[k]
                #
                #    print(filelist[k],'\n',url2Dwl, '\n',outFile)
                #
                #    filename = wget.download(url = url2Dwl,out = outFile)
...
path = 'C:\\Users\\Vinicius_Note\\Desktop\\Scripts\\Integration\\BETA\\'

gpm_month_download(path,'2018-01-01','2018-11-01')
