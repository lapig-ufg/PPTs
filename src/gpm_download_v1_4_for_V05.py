    ...
    try:
        for i in range(0,len(years),1):
            ...
            for i in range(0,len(filelist),1):
                ...
                if filelist[i] in os.listdir(input_dir):
                    
                    if int(os.path.getsize(input_dir+backslh+filelist[i])) == file_size:
                        #print 'O arquivo ' + filelist[i] + ' ja existe em seu diretorio de armazenamento GPM_BRUTO e se assemelha ao mesmo arquivo do FTP de download!'
                        pass
                    else:
                        print ('O arquivo ' + filelist[i] + ' ja existe em seu diretorio de armazenamento GPM_BRUTO, porem parece estar corrompido, incompleto ou desatualizado! Baixando novamente...\n')
                        #Download message
                        print ("Downloading: %s Bytes: %s MB" % (filelist[i], str(round(float(file_size)/1000000,2))))
                        
                        #Acess file and download
                        remotefile = urlopen(url + filelist[i])
                        localfile = open(input_dir + backslh +filelist[i],'wb')
